...
TEST(navigation_has_existing_key) {
    ...
    auto status = store.set<int64_t>("nav/key1", 123);
...
TEST(navigation_keys_returns_immediate_children) {
    ...
    akasha::Store store;
    (void)store.load("nav", temp.path(), akasha::FileOptions::create_if_missing);
    (void)store.set<int64_t>("nav/a", 1);
    (void)store.set<int64_t>("nav/b", 2);
    (void)store.set<int64_t>("nav/c/nested", 3);
...
TEST(navigation_datasetview_navigation) {
    ...
    (void)store.set<int64_t>("app/settings/retries", 5);
